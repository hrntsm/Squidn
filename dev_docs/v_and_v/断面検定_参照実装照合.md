...
| A12 | S梁 曲げ・von Mises・横補剛数・たわみ | ✅ 一致（**2026-07-22 追補4**: 軸力＋二軸曲げ・qz せん断・fb 新基準を拡張） | `beam.rs`（`check_beam`） |
...
- **C 係数の直接入力は未対応**: `steel_lateral_buckling_c` は端部モーメント比
  ...
  指定値の上書き）には対応していない。
- **角形鋼管の角部外半径 `r=2.5t` は仮定値**: 冷間成形角形鋼管の角部外半径を
  実断面値ではなく代表的な仮定値（`r=2.5t`）で統一している。実際の角部半径は
  製品規格により異なるため、精算が必要な場合は断面データへの角部半径追加と
  あわせた見直しが必要。
...
- **座屈長さは強軸・弱軸の区別なく `lk` 単一**: 圧縮時の `fc` 算定における
  細長比 `λ=lk/i_min` は、`ctx.lk`（未指定時は部材長）を強軸・弱軸で共通の
  座屈長さとして用いる（`i_min=√(min(Iy,Iz)/A)`、column.rs と同じ流儀）。
  強軸・弱軸で異なる座屈長さを個別に指定する仕組みは無い。
