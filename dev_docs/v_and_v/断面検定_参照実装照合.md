...
| A11 | S造 F値表・ft/fs/fc・fb（C係数含む） | ✅ 一致（**2026-07-22 追補5**: fc の細長比 λ を強軸・弱軸個別の座屈長さ lk_y/lk_z で評価するよう拡張） | `steel.rs:63`（F値）, `steel.rs:171`（fc）, `steel.rs:214`（fb）, `steel.rs:258`（C） |
| A12 | S梁 曲げ・von Mises・横補剛数・たわみ | ✅ 一致（**2026-07-22 追補4**: 軸力＋二軸曲げ・qz せん断・fb 新基準を拡張。**追補5**: 角形鋼管せん断の角部半径 r を断面入力値化、λ を lk_y/lk_z 個別評価に変更） | `beam.rs`（`check_beam`） |
| A13 | S柱 軸+2軸曲げ・せん断 | ✅ 一致（**2026-07-22 追補5**: λ を lk_y/lk_z 個別評価に変更） | `steel.rs:600`（`check_column`） |
...
- **C 係数の直接入力は未対応**: `steel_lateral_buckling_c` は端部モーメント比
  `M2/M1` から自動算定するのみで、参考資料が認める C の直接入力（ユーザー
  指定値の上書き）には対応していない。
- **角形鋼管の角部外半径 `r=2.5t` は仮定値**: **対応済み（2026-07-22 追補5）**。
  詳細は追補5参照。
- **柱の検定は旧基準 fb のまま**: `SteelFbRule::New`（AIJ-ASD19 の fb）は
  `check_beam` にのみ配線されており、`check_column`（`column.rs`）は引き続き
  旧基準の `fb1`/`fb2` を用いる。柱への新基準適用は本改修のスコープ外。
...
- **柱の座屈長さ自動算定（K・h）は両軸同値のまま**: `app/actions.rs`・
  `mcp/job/design_check.rs` の `K` は現状 1 方向の剛度比のみから算定しており、
  `lk_y`/`lk_z` の両方に同じ値（K・h）を設定している。強軸・弱軸で異なる
  座屈長さ係数 K を個別に算定する精緻化は将来課題。
- **`lk_y`/`lk_z` の直接入力 UI は未提供**: `SteelDesignAttr` 等のユーザー入力経路
  に軸別座屈長さの直接指定欄は無く、現状は柱の自動算定（K・h、両軸同値）または
  既定値（部材長）のみが使われる。
