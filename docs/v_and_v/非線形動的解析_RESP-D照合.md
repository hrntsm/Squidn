...
- **制振要素の残装置:** §8 でマクスウェル要素（`ElementKind::Damper`）と時刻歴配線を、
  §10 で GUI 作成・編集・削除を実装。残るは各社ダンパーの製品固有則（SUB、アンボンド
  ブレース、TRC、ユニットゴム修正 HD、RDT、筒状流体/粘性体、GRAST 3 要素）。多くは
  §8 のマクスウェル・§4 の辻山田・バイリニア/トリリニア骨格の組合せで表現でき、製品係数を
  `DamperProps` に拡張して対応する。JFE LY トリリニアもここに含む。
