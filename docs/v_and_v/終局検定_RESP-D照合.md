...
- `ultimate/rc_shear_ductility.rs`: μ・ν・λ・tanθ の手計算照合と (6.4.10)/(6.4.11) の境界連続性、
  Vu=min(Vu1,Vu2,Vu3) 手計算照合、軽量 0.9 倍、引張軸力でアーチ消失、不正入力→0（計 9 件）。
- `ultimate/tests.rs`: ドライバの柱・梁判別、各耐力が正、軽量低減、RcRect 以外スキップ、
  付着 OFF、2 軸曲げ余裕度（rx²+ry²=1 で余裕度=1.0）、せん断式切替（塑性↔靭性）（計 6 件）。

## 未実装・今後の課題（原典照合リスト）

RESP-D「06 終局検定」のうち、本照合で**未実装**の項目:

1. **靭性指針式の付着考慮せん断信頼強度 Vbu**（`Vbu=min(Vbu1,Vbu2)`、6.8.14-16）。せん断
   信頼強度 Vu（§2-b）は実装済み。Vbu は 2 段目主筋周長 Σφ₂・降伏ヒンジ計画の区別など
   付着設計側の入力が必要なため今後の課題。
