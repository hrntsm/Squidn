...
| **断面形状**（`Standard`） | ST-Bridge 標準の断面要素（`StbSecColumn_S` 等）＋形鋼ライブラリ（`StbSecSteel`）で表す | BIM・他社ソフトへの受け渡し | **不可**（下記参照） |
...
> **重要（断面形状モードの往復不可）**: 断面形状モードで書き出したファイルを Squid-N で読み込むと、
> 現行のインポータは標準断面要素（`StbSecColumn_S` 等）を解釈しないため断面が復元されず、
> 部材の断面参照が壊れる（ダングリング参照）。この状態はモデル検証（`validate`）に通らないため、
> GUI の「ST-Bridge 読込」では**取り込みが拒否される**（＝実質 Squid-N では読み戻せない）。
> Squid-N で再利用するモデルは**物性モード**か `.scz` で保存すること。断面形状モードは
> 他ソフト・BIM への一方向（outbound）出力専用である。
